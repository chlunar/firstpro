...
class Planet:
    ...
    def update_location(self, t):
        ...
        if t < -2 * math.pi / self.angular_velocity:
...
class Moon(Planet):
    ...
    def coordinates(self, t):
        screen_dim = self.canvas.winfo_width()
        y = self.earth.scaled_distance * math.sin(self.earth.angular_velocity * t) + \
            self.scaled_distance * math.sin(self.angular_velocity * t) + screen_dim / 2
        x = self.earth.scaled_distance * math.cos(self.earth.angular_velocity * t) + \
            self.scaled_distance * math.cos(self.angular_velocity * t) + screen_dim / 2
        return (x, y)
